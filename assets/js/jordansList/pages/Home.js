...
export default class Home extends Component {
    ...
    return this.state.categoriesData.map((item, i) => {
      return (
        <div className='category' key={i}>
          <a href={`/${match.params.city}/${item.title}`} className='title'>{item.title}</a>
          <div className='link-section'>

            {this.loopGroups(item)}
  ...
  loopGroups = (category) => {
    ...
    return category.listings.map((item, i) => {
      return (
        <a href={`/${match.params.city}/${category.title}/${item.slug}`} key={i} className='link'>{item.name}</a>
      )
    })
  }
      ...
      <div className='home'>
        <div className='container'>
          ...
          <section className='trending'>
            <input type='text' name='search' className='search' placeholder='Search Classifieds, Jobs, Housing, Discussions, Personals...'/>
            <div className='title'>
